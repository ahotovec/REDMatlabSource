function [varargout]=sac2RaPickswinston(year,dayvec,varargin)
...
for k=1:length(dayvec)
    ...
    for m=1:length(stationList)
        
        temp        = stationList(m);
        saveName    = textscan(temp{1},'%s','Delimiter','.');
        saveName    = saveName{1}(1);
        
        sprintf('Picking through station %s',char(staname))
        ...
        numSec  = ( S(1).recSampInt )*( S(1).recNumData );
