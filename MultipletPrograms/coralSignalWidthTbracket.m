function [tau,varargout] = coralSignalWidthTbracket(S,t1,t2)
...
% This differs from regular coralSignalWidth because you define the time
% interval over which to calculate the signal width
%
% USAGES
...
% t1:   the start time of the signal over which to calculate signal width (sec)
% t2:   the end time of the signal over which to calculate signal width (sec)
...
%
% 25 Sep 2012
% Kate Allstadt edited to allow you to calculate signal width only for a
% desired interval
%-----------------------------------------------------------------------

%cut S to desired interval
opt.cutType='absTime';
opt.absStartTime=timeadd([S.recStartTime],t1);
opt.absEndTime=timeadd([S.recStartTime],t2);
S=coralCut(S,opt);
