function [D] = coralRaStackNoZero(S,varargin)
% Same as coralRaStack but does not include flatlined data
...
%find flatlined data and replace zeros with NaN so it doesn't get counted
%in median
if any(find(max([S(:).data])-min([S(:).data])<1e-10))
    ind=find(max([S(:).data])-min([S(:).data])<1e-10);
    for b=1:length(ind)
        S(ind(b)).data=NaN.*S(ind(b)).data;
    end
end
